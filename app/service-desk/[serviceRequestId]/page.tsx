...
export default async function Page({ params, searchParams }: { params: Promise<{ serviceRequestId: string }>; searchParams?: Promise<{ error?: string; warning?: string }> }) {
  ...
  const { data: communications } = await supabase.from("service_request_communications" as never).select("id,channel,direction,communication_type,recipient_email,status,created_at").eq("organization_id", item.organization_id).eq("service_request_id", item.id).order("created_at", { ascending: true }).order("id", { ascending: true });

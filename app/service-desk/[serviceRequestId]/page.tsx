...
export default async function Page({ params, searchParams }: { params: Promise<{ serviceRequestId: string }>; searchParams?: Promise<{ error?: string; warning?: string }> }) {
  const [{ serviceRequestId }, data, access, query] = await Promise.all([params, getLiveOrganizationData(), getAccessContext(), searchParams ?? Promise.resolve({ error: undefined as string | undefined, warning: undefined as string | undefined })]);
  ...
  const { data: communications } = await supabase.from("service_request_communications" as never).select("id,channel,direction,communication_type,recipient_email,status,created_at").eq("organization_id", item.organization_id).eq("service_request_id", item.id).order("created_at", { ascending: false }).order("id", { ascending: false });
  ...
  const { error: replyError, warning: replyWarning } = query;
  return <>
    <PageHeader eyebrow="Customer Service" title={item.subject} description={item.request_number} action={<div className="detail-badges"><Badge value={item.status} /><Badge value={item.priority} /></div>} />{replyWarning && <div className="form-alert" role="status">{replyWarning}</div>}
    ...
    <section className="panel detail-section service-request-communications"><h2>Communications</h2>{((communications as unknown as Array<{id:string;channel:string;direction:string;communication_type:string;recipient_email:string|null;status:string;created_at:string}> | null) ?? []).length ? (communications as unknown as Array<{id:string;channel:string;direction:string;communication_type:string;recipient_email:string|null;status:string;created_at:string}>).map((communication) => <article key={communication.id}><strong>{communication.channel} · {communication.communication_type.replaceAll("_", " ")}</strong><span>{communication.recipient_email ?? (communication.direction === "INBOUND" ? "Customer message received" : "Portal message sent")} · {new Date(communication.created_at).toLocaleString()}</span><b className="badge">{communication.status}</b></article>) : <p className="muted">No communications recorded yet.</p>}</section>

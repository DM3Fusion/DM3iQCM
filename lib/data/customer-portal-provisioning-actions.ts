...
export async function manageCustomerPortalAccessAction(form: FormData) {
  ...
  let authUser = await authUserByEmail(admin!, email);
  if (!authUser) {
  ...
  const existing = await supabase.from("customer_portal_users").select("id").eq("organization_id", org.id).eq("customer_id", customerId).eq("user_id", authUser.id).maybeSingle();

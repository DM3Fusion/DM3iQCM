...
export async function getCustomerPortalContext(): Promise<CustomerPortalContext | null> {
  ...
  const base = { userId: user.id, activeLinkCount: activeLinks.length, selectedPortalAccessId: selected ?? null, resolvedAccessId: access.id, resolvedOrganizationId: access.organization_id, resolvedCustomerId: access.customer_id, organizationError: Boolean(organizationError), organizationFound: Boolean(organization), organizationStatus: organization?.status ?? null, customerError: Boolean(customerError), customerFound: Boolean(customer), customerStatus: customer?.status ?? null, settingsError: Boolean(settingsError), settingsFound: Boolean(settings), rawPortalEnabled: settings?.portal_enabled ?? null, rawPortalSubmissionEnabled: settings?.portal_submission_enabled ?? null, rawPortalShowPriority: settings?.portal_show_priority ?? null };

...
export async function getCustomerPortalContext(): Promise<CustomerPortalContext | null> {
  ...
  const [{ data: organization }, { data: customer }, { data: settings }] = await Promise.all([
    supabase.from("organizations").select("*").eq("id", access.organization_id).eq("status", "ACTIVE").maybeSingle(),
    supabase.from("customers").select("*").eq("id", access.customer_id).eq("organization_id", access.organization_id).eq("status", "ACTIVE").maybeSingle(),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (supabase as any).from("organization_settings").select("portal_enabled,portal_submission_enabled,portal_show_priority").eq("organization_id", access.organization_id).maybeSingle(),
  ]);
